...
function completeContractOpsDeploymentReview() {
  const scp = contractOps();
  fireEvent.click(scp.getByRole('button', { name: 'Confirm contract specs' }));
  fireEvent.click(scp.getByRole('button', { name: 'Confirm feature mapping' }));
  fireEvent.change(scp.getByLabelText('Admin wallet'), {
    target: { value: connectedWalletAddress },
  });
  fireEvent.click(scp.getByRole('button', { name: 'Save admin wallet' }));
}

function acknowledgeContractOpsProductSetupWarnings() {
  const proceedButton =
    screen.queryByRole('button', { name: 'Record proceed decision' }) ??
    screen.queryByRole('button', { name: 'Proceed with warnings' });
...
describe('App Blockchain Engineer Bot panel', () => {
  it('shows the initial Copilot answer before asking and then renders a backend answer', async () => {
    ...
    expect(screen.getByLabelText('Needs your review')).toBeVisible();
    expect(screen.getByLabelText('Ask ZiLi-OS')).toBeVisible();
    expect(screen.getByLabelText('Tokenisation lifecycle tabs')).toBeVisible();
    expect(screen.getByRole('heading', { name: 'Overview' })).toBeVisible();
    expect(screen.queryByLabelText('ZiLi-OS Copilot workspace')).not.toBeInTheDocument();
    expect(screen.queryByText(/Uses Advisor Bot and Engineering Bot behind the scenes/i)).not.toBeInTheDocument();
    ...
    expect(screen.getByLabelText('ZiLi-OS Copilot composer')).toBeVisible();
    ...
    const botComposer = screen.getByRole('textbox', { name: 'ZiLi-OS Copilot' });
    ...
    expect(screen.getByTestId('engineer-answer')).toHaveTextContent('Routing your message with the current Overview context...');
    ...
    expect(
      screen
        .getByLabelText('ZiLi-OS console')
  ...
  it('routes explanation questions to Advisor without exposing a mode toggle or mutating Product Setup', async () => {
    ...
    expect(screen.getAllByLabelText('ZiLi-OS routing').at(-1)).toHaveTextContent('Explainer');
  ...
  it('uses shared lifecycle state for the Investor Wallets tab, snapshot, vault, and Contract Ops handoff', () => {
    ...
    expect(within(screen.getByTestId('smart-contract-control')).getByRole('heading', { name: 'Contract Ops' })).toBeVisible();
  ...
  it('captures subscription and redemption parameters as working shared lifecycle state', () => {
    ...
    expect(screen.getByLabelText('Lifecycle snapshot')).toHaveTextContent('Subscription');
  ...
  it('completes the golden lifecycle flow into wallet-intent review and locked operations without execution claims', async () => {
    ...
    expect(screen.getByLabelText('Contract Ops launch readiness')).toHaveTextContent('4 launch blocker(s)');
    expect(
      screen.getAllByText(
        'Contract Ops exposes Record NAV Event, Whitelist Wallet, and Allocation / Mint when their wallet, deployment, ABI, parameter, and evidence gates are satisfied. Other operations need explicit adapters before release.',
      ).length,
    ...
    expect(screen.getByLabelText('Generated smart contract artifacts')).toHaveTextContent('Not detected');
    expect(screen.getByLabelText('Generated smart contract artifacts')).toHaveTextContent('No wallet address');
    expect(screen.getByLabelText('Contract Ops evidence summary')).toHaveTextContent('No deployment evidence yet');
    ...
    expect(screen.getByLabelText('Generated smart contract artifacts')).toHaveTextContent('No contract address');
    const rightRail = screen.getByLabelText('ZiLi-OS console');
    ...
    expect(screen.queryByRole('button', { name: /^(sign|sign now|ready for signature)$/i })).not.toBeInTheDocument();
    ...
    expect(screen.queryByRole('button', { name: /^Wallet Connected on Sepolia$/i })).not.toBeInTheDocument();
  ...
  it('connects a mocked Sepolia wallet from the central workflow surface without unlocking execution', async () => {
    ...
    expect(screen.getByLabelText('Generated smart contract artifacts')).toHaveTextContent('Wallet-signed deployment');
    expect(screen.getByLabelText('Deploy to Sepolia')).toHaveTextContent('Deployment status');
    expect(screen.getByLabelText('Deploy to Sepolia')).toHaveTextContent('Deployment execution not started');
  ...
  it('requests wallet-signed Sepolia deployment from the central workflow and renders provider-returned identifiers only after responses', async () => {
    ...
    render(<App />);
    await completeSmartContractPreparation();
    completeContractOpsDeploymentReview();
    ...
    expect(screen.getByLabelText('Generated smart contract artifacts')).toHaveTextContent('Local-session evidence');
    expect(screen.getByLabelText('Contract Ops launch readiness')).toHaveTextContent('0 launch blocker(s)');
    ...
    expect(within(screen.getByLabelText('ZiLi-OS console')).queryByRole('button', { name: /deploy/i })).not.toBeInTheDocument();
  ...
  it('records the first wallet-signed NAV event from Contract Ops only after confirmed deployment evidence', async () => {
    ...
    expectCopilotActionsNotToContainButton(/Record NAV/i);
    expect(within(screen.getByLabelText('ZiLi-OS console')).queryByRole('button', { name: /Record NAV/i })).not.toBeInTheDocument();
  ...
  it('whitelists a target wallet from Contract Ops only after explicit target input and confirmed deployment evidence', async () => {
    ...
    expect(within(screen.getByLabelText('ZiLi-OS console')).queryByRole('button', { name: /Whitelist/i })).not.toBeInTheDocument();
    ...
    within(screen.getByTestId('smart-contract-control'))
      .getAllByRole('button', { name: 'Submit Allocation / Mint' })
  ...
  it('blocks wallet-signed deployment when the wallet is wrong-chain or rejects submission', async () => {
    ...
    expect(screen.getByLabelText('Generated smart contract artifacts')).toHaveTextContent('No transaction hash.');
    expect(screen.getByLabelText('Contract Ops evidence summary')).toHaveTextContent('Deployment evidence: Deployment Evidence: Rejected in wallet');
  });
  ...
  it('supports navigation-only left rail, persistent ZiLi-OS console, and a non-dashboard brief artifact surface', () => {
    render(<App />);

    const leftRail = screen.getByLabelText('Project navigation');
    const rightRail = screen.getByLabelText('ZiLi-OS console');
    expect(within(leftRail).getByText('Workspace')).toBeVisible();
    expect(within(leftRail).getByText('Engineering')).toBeVisible();
    expect(within(rightRail).getByLabelText('Needs your review')).toBeVisible();
    expect(within(rightRail).getByLabelText('Ask ZiLi-OS')).toBeVisible();
    ...
    expect(within(rightRail).queryByRole('heading', { name: 'Workspace Status' })).not.toBeInTheDocument();
    expect(within(rightRail).queryByRole('heading', { name: 'Capability Status' })).not.toBeInTheDocument();
    expect(within(rightRail).queryByRole('heading', { name: 'Product Vault' })).not.toBeInTheDocument();
    ...
    expect(screen.queryByLabelText('ZiLi-OS console')).not.toBeInTheDocument();
