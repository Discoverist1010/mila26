...
test('guided beta journey creates requirements and exposes Engineering Brief action', async ({ page }) => {
  ...
  await expect(generatedArtifacts.getByText('No transaction hash. No contract address. Deployment state is local-session-only until Track 14C evidence linkage.')).toBeVisible();
