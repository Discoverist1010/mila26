...
The next smart-contract product direction is a predefined subscription-redemption template with configurable stablecoins, subscription window, redemption window, redemption wallet, payout-per-token amount, and redemption delay. Template parameter capture should come before allocation/mint.
