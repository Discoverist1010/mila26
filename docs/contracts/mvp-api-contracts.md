...
| `Mila26LlmRequest` / `Mila26LlmResponse` | Backend-only provider adapter boundary. | Implemented mock and OpenAI boundary. | Track 6A mock, Track 6B real provider. | `server/llm/types.ts`. | Config, factory, and provider tests exist. | See `llm-provider-contract.md`; product routes remain deterministic until Track 6C. |
