...
| `Mila26LlmRequest` / `Mila26LlmResponse` | Backend-only provider adapter boundary. | Implemented mock boundary. | Track 6A mock, Track 6B real provider. | `server/llm/types.ts`. | Config and provider tests exist. | See `llm-provider-contract.md`; no SDK or real provider yet. |
